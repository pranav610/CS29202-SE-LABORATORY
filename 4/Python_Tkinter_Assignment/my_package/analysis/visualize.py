...
def plot_visualization(image, bboxes, pred_class, pred_masks, pred_score, clicked): # Write the required arguments
  ...
  image1 = Image.fromarray(image) # convert the image to PIL format
  image2 = Image.fromarray(image) # convert the image to PIL format

  font = ImageFont.truetype("./arial.ttf", 20) # create a font object
  color = ["red", "green", "blue"] # create a list of colors
  
  for box in range(numBBoxes):
    draw = ImageDraw.Draw(image1) # create a draw object
    ...
    image2 = Image.blend(image2, masked, 0.25) # blend the image with the mask

  image1.save('./output/imgs/Bounding-box_out.png') # save the image
  image2.save('./output/imgs/Segmentation_out.png') # save the image
